...
class CrossHatchingR:
    ...
    def CalculateSpacesIndex(self):
        """
        Calculates the empty cell spaces locations, saving them into a list
        @return: 2d list of spaces
        """
        self._spacesIndex = [ ]
    ...
    def loadPriority(self):
        ...
        logging.debug(
            f"Priority: {self._priorityCell}")  # Notes:{self._notes[self._priorityCell[0]][self._priorityCell[1]]}")
...
print(f"Average Time: {time / (success + fails)}")
